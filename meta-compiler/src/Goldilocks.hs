...
module Goldilocks 
  ( Goldilocks(..)
  , unGoldilocks , goldilocksAsInteger
  , toGoldilocks
  ) 
  where
...
goldilocksPrime = 18446744069414584321

--------------------------------------------------------------------------------

-- | The Goldilocks prime field with @p = 2^64 - 2^32 + 1@
newtype Goldilocks = MkGoldilocks Integer deriving (Eq,Show)

unGoldilocks :: Goldilocks -> Integer
unGoldilocks (MkGoldilocks x) = x

goldilocksAsInteger :: Goldilocks -> Integer
goldilocksAsInteger = unGoldilocks

toGoldilocks :: Integer -> Goldilocks
toGoldilocks = MkGoldilocks . modGoldilocks

modGoldilocks :: Integer -> Integer
modGoldilocks x = mod x goldilocksPrime

instance Num Goldilocks where
  fromInteger = toGoldilocks . fromInteger
  negate (MkGoldilocks x)                  = MkGoldilocks $ neg goldilocksPrime x
  (+)    (MkGoldilocks x) (MkGoldilocks y) = MkGoldilocks $ add goldilocksPrime x y
  (-)    (MkGoldilocks x) (MkGoldilocks y) = MkGoldilocks $ sub goldilocksPrime x y
  (*)    (MkGoldilocks x) (MkGoldilocks y) = MkGoldilocks $ mul goldilocksPrime x y  
  abs    x = x
  signum _ = toGoldilocks 1  

instance Fractional Goldilocks where
  recip (MkGoldilocks x)                  = MkGoldilocks $ inv goldilocksPrime x
  (/)   (MkGoldilocks x) (MkGoldilocks y) = MkGoldilocks $ div_ goldilocksPrime x y
